...
public:
    OpenPoseDepth()
    {
        ...
        depth_image.subscribe(nh_, "/camera/aligned_depth_to_color/image_raw", 1);
            ...
            for(int person = 0; person<human_list_msg->human_list.size();person++){
                ...
                for(int bodyPart=0; bodyPart<human.body_key_points_with_prob.size(); bodyPart++){
                    ...
                    if(p>0){
                        ...
                        else if(bodyPart!=11 && bodyPart!=14 && bodyPart!=19 && bodyPart!=20 && bodyPart!=21 && bodyPart!=22 && bodyPart!=23 && bodyPart!=24){
